...
public class PricePlanService {
    ...
    public BigDecimal calculateCostForPricePlanId(List<ElectricityReading> electricityReadings, String pricePlanId){
        BigDecimal average = calculateAverageReading(electricityReadings);
        BigDecimal timeElapsed = calculateTimeElapsed(electricityReadings);

        BigDecimal averagedCost = average.divide(timeElapsed, RoundingMode.HALF_UP);
        BigDecimal unitRate = pricePlans.stream().filter(pricePlan -> pricePlan.getPlanName().equalsIgnoreCase(pricePlanId)).findFirst().get().getUnitRate();
